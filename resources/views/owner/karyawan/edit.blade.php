...
<div class="form-card">
    <form action="{{ route('owner.karyawan.update', $karyawan->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="nama" name="nama" class="form-control" value="{{ $karyawan->nama }}" required>
            </div>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="nik" name="nik" class="form-control" value="{{ $karyawan->nik }}">
            </div>
            
            <div class="form-group">
                <label for="no_telepon">Nomor Telepon</label>
                <input type="text" id="no_telepon" name="no_telepon" class="form-control" value="{{ $karyawan->no_telepon }}">
            </div>
            ...
            <div class="form-group">
                ...
                <select id="cabang_id" name="cabang_id" class="form-control" required>
                    @foreach($cabangs as $c)
                        <option value="{{ $c->id }}" {{ $karyawan->cabang_id == $c->id ? 'selected' : '' }}>{{ $c->nama_cabang }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-grid">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                <div class="form-group">
                    ...
                    <input type="time" id="jam_masuk_shift" name="jam_masuk_shift" class="form-control" value="{{ substr($karyawan->jam_masuk_shift, 0, 5) }}" required>
                </div>
                <div class="form-group">
                    <label for="jam_pulang_shift">Jam Pulang</label>
                    <input type="time" id="jam_pulang_shift" name="jam_pulang_shift" class="form-control" value="{{ substr($karyawan->jam_pulang_shift, 0, 5) }}" required>
                </div>
            </div>

            <div class="form-group">
                <label for="gaji_pokok">Gaji Pokok (Rp)</label>
                <input type="number" id="gaji_pokok" name="gaji_pokok" class="form-control" value="{{ intval($karyawan->gaji_pokok) }}" min="0" required>
            </div>

            <div class="form-group">
                <label for="upah_lembur_per_jam">Upah Lembur per Jam (Rp)</label>
                <input type="number" id="upah_lembur_per_jam" name="upah_lembur_per_jam" class="form-control" value="{{ intval($karyawan->upah_lembur_per_jam) }}" min="0" required>
            </div>
        </div>
        ...
        <div class="form-group">
            @if($karyawan->foto_wajah)
                <div style="margin-bottom: 1rem;">
                    <p style="font-size: 0.85rem; font-weight: 600; margin-bottom: 0.5rem; color: #475569;">Foto Terdaftar Saat Ini:</p>
                    <img src="{{ asset($karyawan->foto_wajah) }}" alt="Foto Saat Ini" style="width: 120px; height: 120px; border-radius: 0.5rem; object-fit: cover; border: 2px solid #e2e8f0;" onerror="this.src='/profiles/default.jpg'">
                </div>
            @endif
            <label for="foto_wajah">Ganti Foto Wajah (Pilih jika ingin mengganti)</label>
            <input type="file" id="foto_wajah" name="foto_wajah" class="form-control" accept="image/*">
            <small class="help-text">Unggah foto baru jika ingin memperbarui wajah referensi Face Recognition karyawan.</small>
        </div>
